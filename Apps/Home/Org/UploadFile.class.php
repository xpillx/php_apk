...
//            	dump($file['name']);
				if (preg_match("/([\x81-\xfe][\x40-\xfe])/", $file['name'])) {    //echo "含有中文";
//                    $name = substr($file['name'] , 0 , strrpos($file['name'], '.'));
//                    $ext = substr($file['name'] , strrpos($file['name'], '.'));
//                    $file['name'] = base64_encode($name ) . $ext;
//					$this->error( '上传文件的文件名不能包含中文');
//					return false;
				}
                //登记上传文件的扩展信息
                $file['extension']  = $this->getExt($file['name']);
    ...
    function thumb($image, $thumbname,  $maxWidth=200, $maxHeight=50 ,$radius = 0) 
    {
        // 获取原图信息
        ...
        if ($info !== false) {
            ...
            $type_arr = array('jpg','jpeg','png','gif','bmp');
            
            $image_type = strtolower(substr($image,strrpos($image,'.')+1));  //原图片类型
            $save_type = strtolower(substr($thumbname,strrpos($thumbname,'.')+1));  //生成图片类型
            if(!in_array($image_type,$type_arr))
            {
            	$this->error("图片名称错误:{$image}");
            	return false;
            }
            if(!in_array($save_type,$type_arr))
            {
            	$this->error("图片名称错误:{$thumbname}");
            	return false;
            }
            unset($info);
            ...
            // 载入原图
            $createFun = 'ImageCreateFrom' . ($image_type == 'jpg' ? 'jpeg' : $image_type);
            $srcImg = $createFun($image);

            //创建缩略图
            if ($image_type != 'gif' && function_exists('imagecreatetruecolor'))
			...
			//处理透明问题
			 if($image_type=="png") {
			            ...
			            $colorTransparent = imagecolorallocatealpha($thumbImg, 0, 0, 0, 127);
			  ...
			  elseif($image_type=="gif") 
			  {
		       ...
		       {
			        Imagefill($thumbImg, 0, 0, imagecolorallocate($thumbImg, 255, 255, 255));
		       }
		       //透明处理结束
...
//                imageinterlace($thumbImg, $interlace);
			...
			{
				if(!mkdir(dirname($thumbname),0777,true))
				{
					$this->error = '无法创建目录：'.dirname($thumbname);
					return false;		
				}
			}
			
			if($save_type == 'png' && intval( $radius) >0)
			{
				$thumbImg = $this->radius($thumbImg,intval( $radius));
			}
			
            // 生成图片
            $imageFun = 'image' . ($save_type == 'jpg' ? 'jpeg' : $save_type);
//            dump(function_exists($imageFun));
//            dump($imageFun);
    ...
    private function getSaveName($filename) {
        ...
        if(empty($rule)) {//没有定义命名规则，则保持文件名不变
            $name = substr($filename['name'] , 0 , strrpos($filename['name'], '.'));
            $ext = substr($filename['name'] , strrpos($filename['name'], '.'));
            $filename['name'] = ($name ) . "(".date('Y-m-d-H-i-s').")" . $ext;
    ...
    private function radius($image,$radius)
    {
    	$topleft = true;
        $bottomleft = true;
        $bottomright = true;
        ...
        $colorTransparent = imagecolorallocatealpha($im_temp, 0, 0, 0, 127);
        imagefill($im_temp, 0, 0, $colorTransparent);
        imagesavealpha($im_temp, true);
		//处理透明结束

		imagecopyresampled($im_temp, $image, 0, 0, 0, 0, $startx, $starty, $width, $height);
		$bg = imagecolorallocatealpha ($im_temp, hexdec('FF'),hexdec('FF'),hexdec('FF'),127);
